...
const UI = {
  switchTab(tab) {
    ['overview', 'onboard', 'rooms', 'residents', 'documents', 'payments'].forEach(t => {
      ...
      document.getElementById(`tab-${t}`).classList.remove('text-pink-theme', 'border-pink-theme');
      document.getElementById(`tab-${t}`).classList.add('border-transparent');
    });
    document.getElementById(`view-${tab}`).classList.remove('hidden');
    document.getElementById(`tab-${tab}`).classList.add('text-pink-theme', 'border-pink-theme');
      ...
      return `
        <div>
          <div class="flex justify-between text-xs mb-1 font-semibold text-stone-600">
            <span>${floor}</span>
            <span>${data.occupied}/${data.total} beds</span>
          </div>
          <div class="w-full bg-stone-100 rounded-full h-2 overflow-hidden">
            <div class="bg-[#522138] h-2 rounded-full transition-all duration-500" style="width: ${pct}%"></div>
          </div>
        </div>
      `;
    }).join('');
  ...
  filterRooms(floor) {
    ...
    ['1', '2', '3', '4'].forEach(f => {
      ...
      if (f === floor) {
        btn.className = "px-5 py-2 rounded-full text-xs font-bold bg-[#522138] text-white";
      } else {
        btn.className = "px-5 py-2 rounded-full text-xs font-bold bg-white border border-stone-200 text-stone-600";
      }
    });
    this.renderRooms();
  },
        ...
        <div class="bg-white border border-stone-200/80 rounded-2xl p-5 shadow-sm space-y-4">
          <div class="flex justify-between items-center border-b border-stone-100 pb-3">
            <div class="flex items-center gap-2">
              ...
              <h3 class="font-bold text-base text-stone-900">${r.roomNumber}</h3>
            </div>
            ${statusPill}
          </div>
          ...
          <div class="grid grid-cols-2 gap-2">
            ${r.occupants.map(o => `
              <div class="p-2.5 bg-stone-50 rounded-xl border border-stone-100">
                <p class="font-bold text-stone-800 text-xs">${o.name}</p>
  ...
  renderResidents() {
    ...
    if (filtered.length === 0) {
      tbody.innerHTML = `<tr><td colspan="7" class="p-6 text-center text-stone-400">No matching residents found.</td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = filtered.map(r => {
      ...
      return `
        <tr class="hover:bg-stone-50/80">
          ...
          <td class="p-4 text-stone-600">${r.roomNumber} · Bed ${r.bed}</td>
          <td class="p-4 text-stone-500 flex items-center gap-1.5">
          ...
          <td class="p-4 text-stone-500">${formatDate(r.joinDate)}</td>
          <td class="p-4 font-bold text-stone-900">₹${Number(r.rent).toLocaleString('en-IN')}</td>
          <td class="p-4">${badge}</td>
          <td class="p-4 text-right">
            <button onclick="vacateResident('${r.id}')" class="text-xs text-stone-500 hover:text-stone-800 font-bold flex items-center gap-1 ml-auto">
  ...
  renderDocuments() {
    ...
    if (State.store.residents.length === 0) {
      container.innerHTML = `<div class="p-8 text-center text-stone-400">No tenants registered yet.</div>`;
      return;
    }
    ...
    container.innerHTML = State.store.residents.map(r => {
      ...
      return `
        <div class="p-4 bg-white rounded-xl border border-stone-200/80 flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-stone-400">Room ${r.roomNumber} · Bed ${r.bed}</p>
            </div>
          </div>
          ...
          <div class="flex items-center gap-3">
            ${hasDoc ? `
              ...
              <a href="${r.aadharFileUrl}" target="_blank" class="px-4 py-1.5 bg-[#522138] text-white rounded-full text-xs font-bold hover:bg-[#3d1829] flex items-center gap-1">
            ...
            ` : `
              ...
              <button onclick="switchTab('onboard')" class="px-4 py-1.5 bg-[#522138] text-white rounded-full text-xs font-bold hover:bg-[#3d1829] flex items-center gap-1">
  ...
  renderPayments() {
    ...
    if (State.store.payments.length === 0) {
      container.innerHTML = `<div class="p-8 text-center text-stone-400">No rent records found.</div>`;
      return;
    }

    container.innerHTML = State.store.payments.map(p => {
      const isPaid = String(p.paid).toUpperCase() === 'TRUE';
      const res = State.store.residents.find(r => r.id === p.residentId) || {};
      ...
      return `
        <div class="p-4 hover:bg-stone-50/80 flex justify-between items-center transition-colors">
          <div>
            ...
            <p class="text-xs text-stone-400 mt-0.5">${roomInfo}</p>
          </div>

          <div class="flex items-center gap-3">
            <span class="font-bold text-stone-900 text-sm">₹${Number(p.amount).toLocaleString('en-IN')}</span>${isPaid ? `
              <span class="text-xs text-stone-400">paid ${p.paidOn ? formatDate(p.paidOn) : ''}</span>
              <span class="bg-emerald-100 text-emerald-800 text-xs px-3 py-1 rounded-full font-bold flex items-center gap-1">
              ...
              <button onclick="downloadReceipt('${resName}', '${roomInfo}', '${p.amount}', '${p.paidOn}', '${p.month}')" title="Download Receipt" class="w-8 h-8 flex items-center justify-center text-stone-500 bg-stone-100 hover:bg-stone-200 rounded-full transition-all">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
              </button>
              <button onclick="openWhatsApp('${phone}', '${resName}', '${p.amount}', '${p.month}', '${p.paidOn}')" title="Send WhatsApp Receipt" class="w-8 h-8 flex items-center justify-center text-emerald-700 bg-emerald-100 hover:bg-emerald-200 rounded-full transition-all">
            ...
            ` : `
              <button onclick="markPaid('${p.residentId}', '${p.month}', ${p.amount})" class="text-xs border border-amber-200 text-amber-800 bg-amber-50 hover:bg-amber-100 px-3.5 py-1.5 rounded-full font-bold transition-all flex items-center gap-1">
